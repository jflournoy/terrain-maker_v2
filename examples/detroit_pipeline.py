...
def parse_args():
    ...
    parser.add_argument(
        '--all',
        action='store_true',
        help='Render all views (mesh built once, reused for all)'
    )
    parser.add_argument(
        '--explain',
        type=str,
        metavar='TASK',
        help='Show execution plan for a task without running'
    )
    ...
    parser.add_argument(
        '--clear',
        action='store_true',
        help='Clear cache and rebuild from scratch'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2048,
        help='Render samples for quality (default: 2048)'
    )
    parser.add_argument(
        '--width',
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.264,
        help='Camera distance multiplier (default: 0.264)'
    )
    parser.add_argument(
        '--elevation',
        type=float,
...
def main():
    ...
    if not args.all:
        ...
        if stats['total_files'] > 0:
            print(f"\n[Cache] Stats:")
            print(f"  DEM cache: {stats['dem']['cache_files']} files, {stats['dem']['total_size_mb']:.1f} MB")
            print(f"  Mesh cache: {stats['mesh']['blend_files']} files, {stats['mesh']['total_size_mb']:.1f} MB")
