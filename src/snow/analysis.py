...
class SnowAnalysis:
    ...
    def calculate_sledding_score(self, stats=None, min_depth_mm=100, min_coverage=0.3):
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ...
            variability_score = 1 - np.clip(
                (stats['interseason_cv'] + stats['mean_intraseason_cv']) / 4, 
                0, 1
            )
        ...
        logger.info(f"  Pixels > 0.7: {high_score_pixels} "
    ...
    def create_dem_features(self, dem_files, snodas_data, snodas_metadata, cache_dir="feature_cache"):
        """
    ...
    # -------------------------------------------------------------------------
    def download_snodas_data(self, start_date, end_date, output_dir=None,
                             masked=False, max_retries=3, max_workers=4, rate_limit=1.0):
        """
        ...
        logger.info(f"Downloading SNODAS from {start_date.date()} to {end_date.date()}, "
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            future_map = {
                executor.submit(self._download_single_date, arg): arg[0]
    ...
    def _store_snow_stats_in_terrain(self, stats, metadata):
        ...
        for key, arr in stats.items():
            ...
            self.terrain.add_data_layer(
                ...
                transform,
                crs,
                target_layer='dem'  # ensures alignment
            )
    ...
    def batch_process_snodas_data(self, extent, processed_dir='processed_snodas', max_workers=14):
        ...
        args_list = [
            (f, extent, processed_dir, (target_height, target_width, target_transform), self.resample_to_extent)
            for f in dat_files
        ]
        processed_files = {}
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            future_map = {
                executor.submit(_process_single_file, arg): arg[0]
                for arg in args_list
            }
    ...
    def calculate_snow_statistics(self, processed_files,
                                  snow_season_start_month=11,
                                  snow_season_end_month=4):
        """
        ...
        for date, filepath in processed_files.items():
            if snow_season_start_month <= date.month <= 12 or 1 <= date.month <= snow_season_end_month:
        ...
        logger.info(f"Computing stats for {len(seasons)} winter seasons, months {snow_season_start_month} to {snow_season_end_month}")
        ...
        for season, filelist in tqdm(seasons.items(), desc="Processing winter seasons"):
            ...
            logger.debug(f"Computed stats for season {season}: shapes -> "
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            final_stats['interseason_cv'] = (
                np.ma.std(seasonal_means, axis=0) / 
                np.ma.mean(seasonal_means, axis=0)
            )
