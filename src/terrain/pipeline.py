...
class TerrainPipeline:
    ...
    def __init__(
        ...
        """
        self.dem_dir = Path(dem_dir) if dem_dir else Path(__file__).parent.parent.parent / "data" / "dem" / "detroit"
        self.cache_enabled = cache_enabled
        self.verbose = verbose

        # Initialize caches
        self.dem_cache = DEMCache(
            cache_dir=Path(dem_cache_dir) if dem_cache_dir else None,
            enabled=cache_enabled
        )
        self.mesh_cache = MeshCache(
            cache_dir=Path(mesh_cache_dir) if mesh_cache_dir else None,
            enabled=cache_enabled
        )
    ...
    def _compute_hash(self, *args, **kwargs) -> str:
        ...
        for key in sorted(kwargs.keys()):
            ...
            if isinstance(val, np.ndarray):
                ...
                parts.append(
                    f"{key}:{hashlib.sha256(json.dumps(val, sort_keys=True, default=str).encode()).hexdigest()}"
                )
            else:
    ...
    def load_dem(self, pattern: str = '*.hgt') -> Tuple[np.ndarray, Affine]:
        ...
        """
        self._log(f"[1/5] Loading DEM from {self.dem_dir}")

        try:
            source_hash = self.dem_cache.compute_source_hash(
                str(self.dem_dir),
                pattern=pattern
            )
            ...
            if cached:
                ...
                self._log(f"      [Cache HIT] Loaded DEM ({dem.shape})")
                return dem, transform
        except Exception as e:
            self._log(f"      [Cache] Load failed: {e}", "debug")

        # Load from source
        from src.terrain.core import load_dem_files
        dem, transform = load_dem_files(str(self.dem_dir), pattern=pattern)
        self._log(f"      [Fresh] Loaded DEM shape: {dem.shape}")
        ...
        try:
            ...
            self._log(f"      [Cached] DEM saved")
        except Exception as e:
            self._log(f"      [Cache] Failed to save: {e}", "warn")
    ...
    def apply_transforms(
        ...
        """
        self._log(f"[2/5] Applying transforms (target_vertices={target_vertices:,})")

        dem, transform = self.load_dem()

        from src.terrain.core import (
            Terrain, reproject_raster, flip_raster, scale_elevation
        )
        ...
        self._log(f"      Zoom factor: {zoom:.6f}")
        ...
        self._log(f"      Downsampled DEM shape: {transformed_dem.shape}")
    ...
    def detect_water(
        ...
        """
        self._log(f"[3/5] Detecting water (threshold={slope_threshold})")
        ...
        self._log(f"      Water detected: {water_pixels:,} pixels ({pct:.1f}%)")
    ...
    def create_mesh(
        ...
        Returns:
            Blender mesh object
        """
        self._log(f"[4/5] Creating Blender mesh")
        ...
        try:
            ...
            if cached_blend:
                self._log(f"      [Cache HIT] Loading mesh from cache")
                # Load mesh from cached blend file
                import bpy
                bpy.ops.wm.open_mainfile(filepath=str(cached_blend))
                for obj in bpy.context.scene.objects:
                    if obj.type == 'MESH':
                        self._log(f"      Loaded cached mesh: {len(obj.data.vertices)} vertices")
                        return obj
        except Exception as e:
            self._log(f"      [Cache] Early check failed: {e}", "debug")

        # Cache miss - run full pipeline with matching params
        dem, transform = self.load_dem()
        water_mask = self.detect_water(
            slope_threshold=water_params['slope_threshold'],
            fill_holes=water_params['fill_holes']
        )
        ...
        if mesh_obj:
            self._log(f"      Mesh created: {len(mesh_obj.data.vertices)} vertices")
    ...
    def render_view(
        ...
        """
        self._log(f"[5/5] Rendering {view} view")

        mesh_obj = self.create_mesh()

        from src.terrain.core import (
            position_camera_relative, setup_light, setup_render_settings,
        ...
        # Setup rendering
        light = setup_light(angle=2, energy=3)
        ...
        if render_file:
            ...
            self._log(f"      Rendered: {render_file.name} ({size_mb:.1f} MB)")
    ...
    def render_all_views(self, views: List[str] = None) -> Dict[str, Path]:
        ...
        for i, view in enumerate(views, 1):
            ...
            try:
                ...
                self._log(f"[✗] Failed to render {view}: {e}", "warn")
    ...
    def clear_cache(self) -> int:
        ...
        self._log(f"Cleared {total} cache files ({dem_deleted} DEM, {mesh_deleted} Mesh)")
