...
@pytest.mark.skipif(HAS_BLENDER, reason="These tests document Blender integration - run inside Blender")
class TestBlenderMeshIntegration:
    """Integration test specifications for Blender-dependent mesh creation.

    These tests document the expected behavior when running inside Blender's Python.
    They are skipped in standard pytest runs because true bpy module isn't available.

    To verify these tests:
    1. Run Blender with: blender --python-expr "import pytest; pytest.main([...])"
    2. Or add this to a Blender add-on test suite

    The tests ensure that when create_mesh() is called within Blender:
    - A mesh object is created with the correct geometry
    - Material nodes are properly configured
    - Vertex positions are correctly calculated with all parameters
    """

    def test_blender_mesh_doc_object_creation_documentation(self):
        """Mesh creation should produce a valid Blender object when Blender is available."""
        dem_data = np.ones((10, 10), dtype=np.float32) * 100
        transform = Affine.identity()
        terrain = Terrain(dem_data, transform)
        ...
        # Create mesh - should return a Blender object
        result = terrain.create_mesh()

        # Verify result is a Blender object
        assert result is not None
        assert hasattr(result, 'name')  # Blender objects have names
        assert hasattr(result, 'data')  # Blender objects have data

    def test_blender_mesh_doc_has_vertices_and_faces(self):
    ...
    def test_blender_mesh_doc_object_naming(self):
        """Created mesh object should have appropriate naming."""
        dem_data = np.ones((10, 10), dtype=np.float32) * 100
        transform = Affine.identity()
        terrain = Terrain(dem_data, transform)
    ...
    def test_blender_mesh_doc_with_boundary_extension(self):
        """Mesh with boundary extension should include side faces."""
        dem_data = np.ones((10, 10), dtype=np.float32) * 100
        transform = Affine.identity()
        terrain = Terrain(dem_data, transform)
        ...
        result = terrain.create_mesh(boundary_extension=True)

        # Should have more vertices and faces than without boundary
        result_with_boundary_verts = len(result.data.vertices)
        result_with_boundary_faces = len(result.data.polygons)

        assert result_with_boundary_verts > 100
        assert result_with_boundary_faces > 100

    def test_blender_mesh_doc_without_boundary_extension(self):
        """Mesh without boundary extension should have fewer faces."""
        dem_data = np.ones((10, 10), dtype=np.float32) * 100
        transform = Affine.identity()
        terrain = Terrain(dem_data, transform)
        ...
        result = terrain.create_mesh(boundary_extension=False)

        # Should still have geometry
        assert len(result.data.vertices) > 0
        assert len(result.data.polygons) > 0

    def test_blender_mesh_doc_centering_affects_geometry(self):
        """Mesh centering should affect vertex positions."""
        dem_data = np.arange(100, dtype=np.float32).reshape(10, 10)
        transform = Affine.identity()
        terrain = Terrain(dem_data, transform)
        ...
        result = terrain.create_mesh(center_model=True)

        # Get vertex positions
        vertices = [v.co for v in result.data.vertices]
        x_positions = [v[0] for v in vertices]

        # Centered model should have x coordinates around 0
        mean_x = np.mean(x_positions)
        assert abs(mean_x) < 5.0  # Should be close to origin

    def test_blender_mesh_doc_height_scaling(self):
        """Different height scales should affect mesh Z coordinates."""
        dem_data = np.ones((10, 10), dtype=np.float32) * 100
        transform = Affine.identity()
        terrain = Terrain(dem_data, transform)
        ...
        result = terrain.create_mesh(height_scale=2.0)

        # Check that vertices exist and have reasonable z values
        vertices = [v.co for v in result.data.vertices]
        z_positions = [v[2] for v in vertices]

        # With height_scale=2.0 on 100 data, z should be ~200
        mean_z = np.mean(z_positions)
        assert 150 < mean_z < 250
